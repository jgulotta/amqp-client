...
object ChannelOwner {
  ...
  def props(init: collection.Seq[Request] = Seq.empty[Request], channelParams: Option[ChannelParameters] = None): Props = Props(new ChannelOwner(init, channelParams))
  ...
  private[amqp] class Forwarder(channel: Channel) extends Actor with ActorLogging {
    ...
    def receive = {
      ...
      case request@CreateConsumer(listener) => {
        ...
        sender() ! withChannel(channel, request)(c => new DefaultConsumer(c) {
...
class ChannelOwner(init: collection.Seq[Request] = Seq.empty[Request], channelParams: Option[ChannelParameters] = None) extends Actor with ActorLogging {
  ...
  def connected(channel: Channel, forwarder: ActorRef): Receive = {
    val _ = channel
    defaultConnected(forwarder)
  }

  def defaultConnected(forwarder: ActorRef): Receive = LoggingReceive {
    case Amqp.Ok(_, _) => ()
    case Record(request: Request) => {
      requestLog :+= request
